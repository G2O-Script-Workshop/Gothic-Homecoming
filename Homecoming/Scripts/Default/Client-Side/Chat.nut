local ChatLine = class
{
	...
	{
		_line.setColor({a = alpha})
	}

	function heightPx()
	{
		return _line.getSizePx().height
	}
...
local ChatPlayerLine = class extends ChatLine
{
	...
	function setPosition(x, y)
	{
		base.setPosition(_nickname.getSizePx().width + x, y)
		_nickname.setPositionPx(x, y)
	}

	function setAlpha(alpha)
	{
		base.setColor({a = alpha})
		_nickname.setColor({a = alpha})
	}

	function heightPx()
	{
		return _nickname.getSizePx().height
	}
